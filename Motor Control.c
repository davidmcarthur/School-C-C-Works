...
#define EN  0x80	// this is key, I missed this last course. I forgot to define my key variables
#define DIR 0x20
...
unsigned int value2;
double fValue;                    // float var used to fix linker error.
...
unsigned char DutyCycle;                  // Duty cycle
...
void main(void) 
{
  ...
  char DCdisplay[4];
  char pwmDisplay[4] = {'O','F','F',' '};
  char dirDisplay[4] = {'C','W',' ',' '};
  ...
  DDRB |= 0x10;                         // Let PB4 becomes OUTPUT
  ...
  PWMPER01 = 600;                  // obtain a period width 1300Hz for Dave's Clock.
  

  for(;;) 
  {
    // ATD Code
    value = ATDDR0;            // Read ATD0 (varistor).  
		ATDSTAT0_SCF = 1;          // clear conv seq complete flag
		ATDCTL5 = 0;               // start a new conversion
		
		// PWM Code
	  DutyCycle = (value-400)/6;                // display value as a percentage of DC
	  
		PWMDTY01 = value;                  // Dty equal to value
	
		
			// Cast variables for display
        (void)sprintf(ADCdisplay,"%2d",value);
        (void)sprintf(DCdisplay,"%2d",DutyCycle);

    // LCD Code
      position(1,0);
      LCDputs("ADC READ: ");
      LCDputs(ADCdisplay); // ADC Value
      position(2,0);
      LCDputs("DUTY CYC:  ");
      position(2,10);
      LCDputs(DCdisplay);         // DC Value, for some reason isn't working well...
      position(2,13);
      LCDputs("%");
      position(3,0);
      LCDputs("PWR:");
    ...
    //Keypad detection
		...
		if (swdataLO & EN) 
		{
		  EN_key ^= 0x01;
			  ...
			  if (EN_key == 0x01) 
			  {
				...
				(void)sprintf(pwmDisplay,"%2s","ON  ");
			  }
			  else 
			  {
				PWME &= ~0x02;                // Off PWM channel 1
				PTT |= 0x80;                  // Off SW4 LED
				(void)sprintf(pwmDisplay,"%2s","OFF ");
			  }
		...
		if (swdataLO & DIR) 
		{
		  ...
		  if (DIR_key == 0x01)
		  {
		    ...
		    (void)sprintf(dirDisplay,"%2s","CCW ");
		  }
		  else 
		  {
		    PORTB &= ~0x10;
		    PTT |= 0x20;                   // Off SW2 LED
		    (void)sprintf(dirDisplay,"%2s","CW  ");
		  }
		}
